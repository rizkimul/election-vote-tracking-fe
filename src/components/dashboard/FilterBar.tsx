  ...
  React.useEffect(() => {
      const fetchFilters = async () => {
          try {
              ...
              const res = await fetch('/api/historical-votes/filters', { headers });
              if (res.ok) {
  ...
  React.useEffect(() => {
      ...
      const fetchKecamatans = async () => {
           try {
              const token = localStorage.getItem('token');
              const headers = { 'Authorization': `Bearer ${token}` };
              const res = await fetch(`/api/historical-votes/filters?dapil=${encodeURIComponent(selectedDapil)}`, { headers });
              if (res.ok) {
                  const data = await res.json();
                  setKecamatans(data.kecamatans || []);
